    ...
    def callback(self, data, camera_name):
        ...
        try:
            ...
            if self.active_goal in names:
                ...
                index = names.index(self.active_goal)
                box = seg_results[0].boxes.xywh[index].cpu().numpy()
                ...
                try:
                    if camera_name == "camera":
                        ...
                        camera_model = self.elevated_camera_model
                    
                    ray = camera_model.projectPixelTo3dRay((obj_x, obj_y))
                    
                    point = PointStamped()
                    point.header = data.header
                    point.point.x = ray[0]
                    point.point.y = ray[1]
                    point.point.z = 0 # on ground
                    
                    # Transform to map frame
                    transformed = self.tf_buffer.transform(point, "map", timeout=rospy.Duration(1.0))
